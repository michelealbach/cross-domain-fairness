...
# Michele Albach
# This file performs 1000 50/50 train test splits on all data from the main survey, but using data without any male survey respondents
...
def load(props):
    file = open("../modifiedData/MainBailNoMales.csv",'r')
    recdata = list(csv.reader(file))
    file.close()
    file = open("../modifiedData/MainUnemNoMales.csv",'r')
    unemdata = list(csv.reader(file))
    file.close()
    file = open("../modifiedData/MainCPSNoMales.csv",'r')
    cpsdata = list(csv.reader(file))
    file.close()
    file = open("../modifiedData/MainHosNoMales.csv",'r')
    hosdata = list(csv.reader(file))
    file.close()
    file = open("../modifiedData/MainLoanNoMales.csv",'r')
    loandata = list(csv.reader(file))
    file.close()
    file = open("../modifiedData/MainInsNoMales.csv",'r')
