...
# Michele Albach
# This file performs one test by training on the data from five domain and testing on the remaining one (from any of the three main surveys depending on commented code)
        ...
        for k in range(int((len(traindata[0])-15)/f)): # For every feature
            ...
            for j in props: # For every property
                ...
                if traindata[i][16+k*f+j] == "Strongly Disagree":
                    ...
                    x.append(3)
                else: # rated as neutral or skipped
                    x.append(0)
            if skip==False:
                X_train.append(x)
...
def CrossDomain5v1(test_domain,props):
...
#print(CrossDomain5v1("Bail",[0,1,2,3,4,5,6,7]))
#print(CrossDomain5v1("Bail",[0,1,2,3,4,5,6]))
