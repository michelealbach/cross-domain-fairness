...
# Michele Albach
# This file performs 1000 50/50 train test splits on all data without caucasians respondents from the survey that removed relevance entirely
...
def load(props):
    file = open("../modifiedData/RemovedBailNoWhites.csv",'r')
    recdata = list(csv.reader(file))
    file.close()
    file = open("../modifiedData/RemovedUnemNoWhites.csv",'r')
    unemdata = list(csv.reader(file))
    file.close()
    file = open("../modifiedData/RemovedCPSNoWhites.csv",'r')
    cpsdata = list(csv.reader(file))
    file.close()
    file = open("../modifiedData/RemovedHosNoWhites.csv",'r')
    hosdata = list(csv.reader(file))
    file.close()
    file = open("../modifiedData/RemovedLoanNoWhites.csv",'r')
    loandata = list(csv.reader(file))
    file.close()
    file = open("../modifiedData/RemovedInsNoWhites.csv",'r')
