...
# Michele Albach
# This file performs 1000 50/50 train test splits using data without male survey respondents from the survey that replaced relevance with increases accuracy
...
def load(props):
    file = open("../modifiedData/ReplacedBailNoMales.csv",'r')
    recdata = list(csv.reader(file))
    file.close()
    file = open("../modifiedData/ReplacedUnemNoMales.csv",'r')
    unemdata = list(csv.reader(file))
    file.close()
    file = open("../modifiedData/ReplacedCPSNoMales.csv",'r')
    cpsdata = list(csv.reader(file))
    file.close()
    file = open("../modifiedData/ReplacedHosNoMales.csv",'r')
    hosdata = list(csv.reader(file))
    file.close()
    file = open("../modifiedData/ReplacedLoanNoMales.csv",'r')
    loandata = list(csv.reader(file))
    file.close()
    file = open("../modifiedData/ReplacedInsNoMales.csv",'r')
