...
# Michele Albach
# This file performs 1000 50/50 train test splits on the data without caucasian respondents from a particular domain from the survey that removed relevance entirely, 
...
def load(domain,props):
    file = open("../modifiedData/Removed"+domain+"NoWhites.csv",'r')
    ...
    for i in range(1,len(data)): # For every respondent
        for k in range(int((len(data[0])-15)/8)): # For every feature
            skip=False
            # append a 1 to Y if rated fair or a 0 if rated unfair
            if data[i][15+k*8] == "Very Fair":
                Y.append(1)
            elif data[i][15+k*8] == "Fair":
                Y.append(1)
            elif data[i][15+k*8] == "Somewhat Fair":
                Y.append(1)
            elif data[i][15+k*8] == "Somewhat Unfair":
                Y.append(0)
            elif data[i][15+k*8] == "Unfair":
                Y.append(0)
            elif data[i][15+k*8] == "Very Unfair":
                Y.append(0)
            ...
            for j in props: # For every property
                ...
                if data[i][16+k*8+j] == "Strongly Disagree":
                    x.append(-3)
                elif data[i][16+k*8+j] == "Disagree":
                    x.append(-2)
                elif data[i][16+k*8+j] == "Somewhat Disagree":
                    x.append(-1)
                elif data[i][16+k*8+j] == "Somewhat Agree":
                    x.append(1)
                elif data[i][16+k*8+j] == "Agree":
                    x.append(2)
                elif data[i][16+k*8+j] == "Strongly Agree":
                    x.append(3)
...
#print(WithinDomain("Ins",[0,1,2,3,4,5,6]))
