...
# Michele Albach
# This file performs 1000 50/50 train test splits on all data from our initial survey
...
def load(props):
    file = open("../../data/MainSurveyBail.csv",'r')
    recdata = list(csv.reader(file))
    file.close()
    file = open("../../data/MainSurveyUnem.csv",'r')
    unemdata = list(csv.reader(file))
    file.close()
    file = open("../../data/MainSurveyCPS.csv",'r')
    cpsdata = list(csv.reader(file))
    file.close()
    file = open("../../data/MainSurveyHos.csv",'r')
    hosdata = list(csv.reader(file))
    file.close()
    file = open("../../data/MainSurveyLoan.csv",'r')
    loandata = list(csv.reader(file))
    file.close()
    file = open("../../data/MainSurveyIns.csv",'r')
...
# Property order for which to skip is [Reliability,Relevance,Privacy,Volitionality,Causes Outcome,Causes Cycle,Causes Disparity,Caused By Group]
#print(AllDomains([0,1,2,3,4,'S',6,7]))
